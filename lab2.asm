...
;
	...
	popa
	popf
    ;jmp     dword ptr CS:[old_41h]  ; В системный обработчик без возврата
...
;
    ...
    mov     AX,3541h    ; Проверить вектор 1C
...
;
    push    DS
;
    lds     DX, CS:old_41h   ; Эта команда эквивалентна следующим двум
;    mov     DX, word ptr old_1C
;    mov     DS, word ptr old_1C+2
    mov     AX,2541h        ; Заполнение вектора старым содержимым
    int     21h
;
...
;============================================================================
    mov AX,3541h                        ;   получить
                                        ;   вектор
    int 21h                             ;   прерывания  1C
    mov word ptr old_41h,BX    ;   ES:BX - вектор
    mov word ptr old_41h+2,ES  ;
    mov DX,offset new_41h           ;   получить смещение точки входа в новый
;                                   ;   обработчик на DX
    mov AX,2541h                        ;   функция установки прерывания
                                        ;   изменить вектор 1C
    int 21h ;   AL - номер прерыв. DS:DX - указатель программы обработки прер.
;
